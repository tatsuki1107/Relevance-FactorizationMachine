...
- 機械学習アルゴリズムのスクラッチ実装
  - MF（Matrix Factorization）とFM（Factorization Machines）を核とする協調フィルタリングアルゴリズムの理解を深めるため、これらの技術を含むアルゴリズムを一から実装した。この実装能力が基盤となり、これから先、新たな技術への理解を深めることができる。

- FMのアルゴリズム効率化
  - `csr_matrix`を使用して、予測と学習のメモリ効率と計算効率を向上させた。
  - FMの予測式の計算効率を高めるために、交互作用の項の計算量を $`O(\text{特徴数} \times \text{因子数})`$に削減した。

- DockerとPoetryの導入
  - 再現性を確保し、チーム全員が同じ結果に基づいてモデルの評価と意思決定を行えるようにした。
  - `slim image`を利用してイメージサイズを減少させ、デプロイの遅延問題に対処した（実験環境ではあるが、本番環境へのデプロイを想像しての対応）。

- Hydraの活用
  - パラメータを構造化し、チームメンバーの理解を容易にすることで開発の生産性を向上させるため。

- プロジェクトドキュメントの詳細化
  - チームメンバー間の認識の齟齬を解消し、プロジェクトの内容を明確に理解する助けをするため。

## 反省ポイント

- 特徴量エンジニアリングと前処理の改善
  - MFの性能向上を大きな目的としてFMを実装したので、最低限の前処理に留まってしまった。例として、欠損値を平均値で補完する、似た特徴量を選定する、データ分割前の標準化などが挙げられる。

- 定性的な評価の不足
  - モデルの変更がもたらす具体的なアウトプットの変化をビジネス面での意思決定をサポートするために効果的に示す必要がある。
