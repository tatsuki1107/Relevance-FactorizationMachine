...
- [実験結果とディスカッション](#実験結果とディスカッション)
  ...
  - [RQ3: $`\hat{L}_{IPS}`$を用いることで過去の露出度合いに依存せずにランキングを生成できるのか？](#rq3-hatl_ipsを用いることで過去の露出度合いに依存せずにランキングを生成できるのか)
  - [RQ4: バイアスのある検証データを使用して、モデル間の性能を評価することは可能か？](#rq4-バイアスのある検証データを使用してモデル間の性能を評価することは可能か)
- [結論](#結論)
- [参考文献](#参考文献)
# 概要  
...
* RQ3: $`\hat{L}_{IPS}`$を用いることで過去の露出度合いに依存せずにランキングを生成できるのか？
* RQ4: バイアスがかかった検証データを使用して、モデル間の性能を評価することは可能か？  
...
### 1. ログデータの生成
「small_matrix.csv」から人工的なログデータを生成する。このデータの評価値行列の値は実験的に集められたため、密度はほぼ100%となっている。本実験では、ログデータのバイアスは露出格差によるものだけであると仮定し、ランダム推薦を行って評価値行列の2%のデータでログデータ$`D`$を生成する。抽出後のログデータサイズは約9万レコードとなる。
...
### 6. データの分割と学習の準備
ログデータ$`D`$はホールド・アウト法を使用し、学習、検証、およびテストデータに分割する。分割の比率は6:2:2とした。テストデータは実環境の性能を模倣する目的で用意しており、教師データとして$`R_{u,i}`$を使用する。これは、オンライン指標（例：CTR）の代替としての役割を果たす。さらに、2つのモデル（MF, FM）を3つの損失$`L_{Ideal}`$, $`\hat{L}_{IPS}`$, $`\hat{L}_{Naive}`$に組み込み、学習を行う。
...
ランキングの精度を重視して、DCG@5をメトリックとして採用する。しかしながら、$`L_{Ideal}`$, $`\hat{L}_{IPS}`$, および $`\hat{L}_{Naive}`$ はそれぞれ異なる検証データを扱うため、使用するチューニングデータも変わる。特に検証データはPosition Based Modelに基づくクリックデータであり、バイアスが考慮された上で$`\hat{L}_{IPS}`$のチューニングが必要である。以下の表は、各推定量におけるチューニング式を示している。
...
具体的に、$`L_{Ideal}`$および$`\hat{L}_{Naive}`$では、検証データにおいてDCG@5を最大化するパラメータを探索する。一方、$`\hat{L}_{IPS}`$では、検証データとテストデータの分布の違いを考慮しながら、DCG@5を最大化する必要がある。  
...
__*画像1: Factorization Machines (FM) に3つの損失を適用した際のテストデータにおける各ランク指標の性能比較__

![Alt text](logs/result/img/DCG_vs_model.png)
__*画像2: Factorization Machines (FM)とMatrix Factorization (MF)の3つの損失に対するDCG@K性能比較__

先ほどのリサーチクエスチョンに基づく実験結果の考察を行う。

### RQ1: FMを使用することにより、$`L_{Ideal}`$の性能は向上し、その結果、$`\hat{L}_{IPS}`$推定量の性能も改善し、実際の応用に適しているのか？

* 画像2に示す通り、Matrix Factorizationを$`L_{Ideal}`$に適用した際、よい性能を示している。しかし、$`\hat{L}_{IPS}`$と$`\hat{L}_{Naive}`$の性能に大きな差が見られなかった。

* 一方、特徴量を取り入れたFMの結果は画像1に示す通り。モデルの複雑性増加により$`L_{Ideal}`$の性能が向上し、バイアスのある状況下でも特徴量が加わったことで$`\hat{L}_{IPS}`$の良好な学習に寄与していると考えられる。

* FMを$`\hat{L}_{Naive}`$に適用した際の性能は、$`\hat{L}_{IPS}`$より劣る結果となった。

* 画像2では、FMとMFのDCG@K性能が示されている。$`L_{Ideal}`$の性能が向上するにつれ、$`\hat{L}_{IPS}`$も良好な性能を示している。

* 研究当初、「MFを$`\hat{L}_{IPS}`$に適用しても性能改善が見られないのは、$`L_{Ideal}`$の性能が悪い」からという仮説を立てていたが結果が少し違った。「不確実なクリックデータしかない状況では真の分布の情報が失われすぎるが故に$`\hat{L}_{IPS}`$と$`\hat{L}_{Naive}`$とで性能に大きな差が見られない」結果となることが明らかになった。FMを用いる場合は確からしい特徴量を扱えるため、MFと比べてよい性能を示していると考えられる。

* 結論として、$`\hat{L}_{IPS}`$にFMを組み込むことによる実用的な有用性の向上が確認された。

![Alt text](logs/result/img/metrics_per_frequency.png)
__*画像3: ログデータにおけるレアアイテムのランク指標性能。Factorization Machines (FM)を取り入れた各推定量の比較。__

### RQ2: FMの採用によって全体的な性能が向上する場合、$`\hat{L}_{Naive}`$が予測できない低露出確率のアイテムの推薦も正確に行われているのか？  

画像3では、レアアイテム集合のFM性能評価を示している。具体的には、露出確率$`\theta_{u,i}`$が0.5以下のアイテムを「レアアイテム」としている。$`\hat{L}_{IPS}`$はレアアイテムにおいても$`L_{Ideal}`$に近い性能を持っている。この結果から、$`\hat{L}_{IPS}`$が低露出アイテムへの適切な学習能力を持っていると解釈できる。  

![Alt text](logs/result/img/mean_exposure.png)
__*画像4: Mean Exposure(ME)@Kを用いた性能比較__

### RQ3: $`\hat{L}_{IPS}`$を用いることで過去の露出度合いに依存せずにランキングを生成できるのか？

通常のランク指標だけでは、IPS推定量が本当に過去の露出に依存せずにランキングを生成できているのか判断できないのだった。そこでカスタム評価指標Mean Exposure@Kを用いた性能比較の結果を画像4に示した。  
* FMを$`\hat{L}_{IPS}`$に適用した場合はどの推薦位置でもアイテムの過去の露出の平均は一定の値を保っている。過去の露出に依存せずに推薦できていると言える。
* FMを$`\hat{L}_{Naive}`$に適用した場合は、__過去に露出が大きかったアイテムほどが高い推薦位置__ に選ばれやすくなっている。
* さらに画像一番右のように、FMとMFをそれぞれ$`\hat{L}_{Naive}`$に適用させた場合を比較すると、MFよりもFMのほうが過去に露出が大きかったアイテムを高い順位に選びやすい結果となった。つまり、__安直にクリックデータを教師データとして、より高性能な機械学習アルゴリズムを学習させると、一部のアイテムへの予測精度しか担保されず、さらにバイアスを増幅させる危険性がある__ ことを示している。
* MFを$`\hat{L}_{IPS}`$に適用した場合、逆に過去の露出が低いアイテムほど高い推薦位置に選ばれやすくなっている。露出バイアス$`\theta_{u,i}`$の影響が強く、$`\hat{L}_{IPS}`$の高分散問題により推論が不安定になっていることが原因でモデルが過信状態にある。$`\theta_{u,i}`$をある下限値でクリッピングすることで多少のバイアスを許容して分散を減らすという策が考えられる。
  
* 結果として、FMを$`\hat{L}_{IPS}`$に適用させることで過去の露出に依存せずより良い推薦をすることができる。

![Alt text](logs/result/img/IPS_estimator_of_DCG.png)  
__*画像5: 検証データを用いた各IPS推定量の性能を比較__

### RQ4: バイアスのある検証データを使用して、モデル間の性能を評価することは可能か？  

モデル間の性能を比較するために、FMとMFの両モデルを学習し、実際の環境を模倣したテストデータでの評価を行った。しかしながら、現実の状況では、モデルの選択とデプロイ後にオンライン評価（例: クリック率）を取得することが一般的である。このため、バイアスのある検証データに基づいてモデルの性能を判断する必要がある。$`\hat{L}_{IPS}`$と$`\hat{L}_{Naive}`$の性能の差は明らかであるため、問題は、$`\hat{L}_{IPS}`$の下で、FMとMFのどちらを選択するかを検証データで評価することになる。本研究では、IPS推定量を使用して$`\hat{L}_{IPS}`$のチューニングを行い、FMとMFの性能を比較した。画像5によれば、FMがMFよりも高い性能を示している。この結果をもとに、FMの採用が性能向上に寄与していると言える。 実際には、定性面の評価も行った上でモデルの選択を判断すると考えられるが、このようにバイアスのある状況下でモデル間の性能の足がかりを掴むことができる。(なお、今回はチューニングを行った検証データのDCG@5の比較を行っているが、より正しく汎化性能を評価するには、もう一つデータセットを作る必要がある。)
